...
test_that("Some infections with imperfect vaccine wrt rel_susceptibility", {
  ...
  state_1[index_S[, 2]] <- state_1[index_S[, 1]]
  state_1[index_S[, 1]] <- 0
  ...
  state_2[index_S[, 2]] <- state_2[index_S[, 1]]
  state_2[index_S[, 1]] <- 0
...
test_that("No symptomatic infections with perfect vaccine wrt rel_p_sympt", {
  ...
  ## None moves into I_P, I_C_1 or I_C_2 ever
...
test_that("None hospitalised with perfect vaccine wrt rel_p_hosp_if_sympt", {
  ...
  ## None hospitalised compartments ever
  ## other than in the 4th age group where some infections are seeded
  ## in the unvaccinated group and because of waning immunity they may
...
test_that("No infections with perfect vaccine wrt rel_infectivity", {
  p <- ZamCovid_parameters(0, rel_infectivity = c(1, 0))
  mod <- ZamCovid$new(p, 0, 1, seed = 1L)
  info <- mod$info()
  ...
  index_I_A <- array(info$index$I_A, info$dim$I_A)
  state[index_I_A[, 2]] <- state[index_I_A[, 1]]
  state[index_I_A[, 1]] <- 0
...
test_that("Returning to unvaccinated stage works for exposed individuals", {
  ...
  ## is stopped after E
  p <- ZamCovid_parameters(0, "england",
                           beta_value = 0,
                           rel_susceptibility = c(1, 0),
                           rel_p_sympt = c(1, 1),
                           rel_p_hosp_if_sympt = c(1, 1),
...
test_that("Vaccine progression through 3 classes works for susceptibles", {
  ...
  vaccine_schedule <- test_vaccine_schedule(daily_doses = Inf,
                                            ...
                                            uptake = 1)
  p <- ZamCovid_parameters(0, region,
                           beta_value = 0,
...
test_that("there are no vaccinated susceptibles when vaccination rate is 0", {
  ...
  expect_equal(s[-seq_len(ZamCovid_n_groups()), , ],
               array(0, c(nrow(s) - ZamCovid_n_groups(), 101)))
})
...
test_that("Can calculate Rt with an (empty) vaccination class", {
  ...
  p <- ZamCovid_parameters(sircovid_date("2020-02-07"), "england",
                           rel_susceptibility = c(1, 1),
                           rel_p_sympt = c(1, 1),
                           rel_p_hosp_if_sympt = c(1, 1),
                           rel_p_death = c(1, 1))
  ...
  np <- 3L
  mod <- ZamCovid$new(p, 0, np, seed = 1L)

  initial <- ZamCovid_initial(mod$info(), 10, p)
  ...
  rt_1 <- ZamCovid_Rt(steps, y[, 1, ], p)
  rt_all <- ZamCovid_Rt_trajectories(steps, y, p)
  ...
  rt_1_single_class <- ZamCovid_Rt(steps, y[, 1, ], p)
  rt_all_single_class <- ZamCovid_Rt_trajectories(steps, y, p)
...
test_that("Effective Rt reduced by rel_susceptibility if all vaccinated", {
  ...
  rt_1_vacc <- ZamCovid_Rt(steps, y_with_vacc[, 1, ], p)
  rt_all_vacc <- ZamCovid_Rt_trajectories(steps, y_with_vacc, p)
...
test_that("N_tots stay constant with vaccination and no waning immunity", {
  ...
  p <- ZamCovid_parameters(0, "london",
...
test_that("N_tot stays constant with vaccination and waning immunity, while
          sero and PCR N_tots are non-decreasing", {
            set.seed(1)
            vaccine_schedule <- test_vaccine_schedule(500000, "london")
            p <- ZamCovid_parameters(0, "london", waning_rate = 1 / 20,
                                     rel_susceptibility = c(1, 0.5, 0.1),
                                     rel_p_sympt = c(1, 1, 1),
                                     rel_p_hosp_if_sympt = c(1, 1, 1),
                                     rel_p_death = c(1, 1, 1),
...
test_that("Outputed S vaccination numbers are what we expect", {
  ...
  p <- ZamCovid_parameters(0, region, waning_rate = 1 / 20,
                           rel_susceptibility = c(1, 0.5),
                           rel_p_sympt = c(1, 1),
                           rel_p_hosp_if_sympt = c(1, 1),
                           rel_p_death = c(1, 1),
...
test_that("build_rel_param works as expected", {
  expect_equal(
    ...
    array(1, dim = c(ZamCovid_n_groups(), 1, 1)))
  mat <- array(rep(c(1, 0.1), each = ZamCovid_n_groups()),
               dim = c(ZamCovid_n_groups(), 1, 2))
  expect_equal(
  ...
  expect_equal(
    ...
    mat)
  mat_rand <- array(c(rep(1, ZamCovid_n_groups()),
                      runif(ZamCovid_n_groups())),
                    dim = c(ZamCovid_n_groups(), 1, 2))
  expect_equal(
...
test_that("build_vaccine_progression_rate rejects insensible inputs", {
  ...
  ntot <- rep(1000, 19)
  expect_error(
    ZamCovid_parameters_vaccination(
      ntot,
      rel_susceptibility = c(1, 1, 1),
...
test_that("run sensible vaccination schedule, catchup = 0", {
  ...
  p <- ZamCovid_parameters(0, "east_of_england",
                           rel_susceptibility = c(1, 1),
                           rel_p_sympt = c(1, 1),
                           rel_p_hosp_if_sympt = c(1, 1),
                           rel_p_death = c(1, 1),
...
test_that("can add vaccination to a set of model state", {
  ...
  p_orig <- ZamCovid_parameters(0, region)
  ...
  p_vacc <- ZamCovid_parameters(0, region,
                                rel_susceptibility = c(1, 1),
                                rel_p_sympt = c(1, 1),
                                rel_p_hosp_if_sympt = c(1, 1),
                                rel_p_death = c(1, 1),
  ...
  mod_orig <- ZamCovid$new(p_orig, 0, 10, seed = 1L)
  mod_vacc <- ZamCovid$new(p_vacc, 0, 10, seed = 1L)
...
test_that("If vaccine dose 2 index provided, we need a schedule", {
  ...
  ntot <- rep(1000, 19)
  expect_error(
    ZamCovid_parameters_vaccination(ntot, vaccine_index_dose2 = 2L),
...
test_that("Can vaccinate given a schedule", {
  p <- ZamCovid_parameters(0, "england", rel_susceptibility = c(1, 1, 0),
                           beta_value = 0,
                           rel_p_sympt = c(1, 1, 1),
                           rel_p_hosp_if_sympt = c(1, 1, 1),
                           rel_p_death = c(1, 1, 1),
...
test_that("Can vaccinate given a schedule with given uptake", {
  region <- "london"
  p <- ZamCovid_parameters(0, region, rel_susceptibility = c(1, 1, 0),
                           beta_value = 0,
                           rel_p_sympt = c(1, 1, 1),
                           rel_p_hosp_if_sympt = c(1, 1, 1),
                           rel_p_death = c(1, 1, 1),
...
test_that("Can catch up on uptake given previous vaccination", {
  ...
  p <- ZamCovid_parameters(sircovid_date("2021-04-10"),
...
test_that("Can catch up on doses not distributed", {
  ...
  mod2 <- ZamCovid$new(p, 0, 1, seed = 1L)
  state <- ZamCovid_initial(info, 1, p)
...
test_that("Can add missing state variables", {
  ...
  mod <- ZamCovid$new(p, 0, 10, seed = 1L)
  info <- mod$info()

  state <- ZamCovid_initial(info, 10, p)
...
test_that("can inflate the number of vacc classes after running with 2", {
  ## two vacc classes
  p1 <- ZamCovid_parameters(0, "east_of_england",
                            rel_susceptibility = c(1, 1))
  ## three vacc classes
  p2 <- ZamCovid_parameters(0, "east_of_england",
                            rel_susceptibility = c(1, 1, 1))

  np <- 4L
  mod1 <- ZamCovid$new(p1, 0, np, seed = 1L)
  initial <- ZamCovid_initial(mod1$info(), 10, p1)
  ...
  expect_equal(sort(y2[, 1], decreasing = TRUE,
                    na.last = FALSE)[-seq_len(info1$len)],
               rep(0, info2$len - info1$len, na.last = FALSE))
...
test_that("Can validate vaccine skip move inputs", {
  expect_error(ZamCovid_parameters(1, "london", rel_susceptibility = c(1, 1, 1),
                                   vacc_skip_to = c(1, 3)),
               "There are 3 vaccine classes so 'vacc_skip_to' must be of")

  expect_error(ZamCovid_parameters(1, "london", rel_susceptibility = c(1, 1),
                                   vacc_skip_weight = c(0, 1, 1)),
               "There are 2 vaccine classes so 'vacc_skip_weight' must be of")

  expect_error(ZamCovid_parameters(1, "london", rel_susceptibility = c(1, 1),
                                   vacc_skip_progression_rate = c(0, 0, 0)),
               "There are 2 vaccine classes so 'vacc_skip_progression_rate'")

  expect_error(ZamCovid_parameters(1, "london", rel_susceptibility = c(1, 1, 1),
                                   vacc_skip_to = c(4, 0, 0)),
               "There are 3 vaccine classes so the values in 'vacc_skip_to'")

  expect_error(ZamCovid_parameters(1, "london", rel_susceptibility = c(1, 1, 1),
                                   vacc_skip_to = c(3, 0, 0),
                                   vacc_skip_weight = c(1, 1, 0)),
               "Require 0 values in 'vacc_skip_weight' for vaccine strata that")

  expect_error(ZamCovid_parameters(1, "london", rel_susceptibility = c(1, 1, 1),
                                   vacc_skip_to = c(3, 0, 0),
                                   vacc_skip_progression_rate = c(1, 1, 0)),
               "Require 0 values in 'vacc_skip_progression_rate' for vaccine")

  expect_error(ZamCovid_parameters(1, "london", rel_susceptibility = c(1, 1, 1),
                                   vacc_skip_to = c(2, 0, 0)),
               "Require vacc_skip_to")

  expect_error(ZamCovid_parameters(1, "london",
                                   rel_susceptibility = c(1, 1, 1, 1),
                                   vacc_skip_to = c(4, 4, 0, 0)),
               "Cannot have more than one vaccine skip move to the same")

  expect_error(ZamCovid_parameters(1, "london", rel_susceptibility = c(1, 1, 1),
                                   vacc_skip_to = c(3, 0, 0),
                                   vacc_skip_weight = c(1.5, 0, 0)),
               "'vacc_skip_weight' must lie in")

  expect_error(ZamCovid_parameters(1, "london",
                                   rel_susceptibility = c(1, 1, 1, 1),
                                   vacc_skip_to = c(3, 4, 0, 0)),
               "Cannot have overlapping vaccine skip moves")
})

test_that("vacc_skip outputs calculated correctly", {
  vaccine_schedule <- test_vaccine_schedule(booster_daily_doses = 10000)

  p <- ZamCovid_parameters(1, "london",
                           rel_susceptibility = rep(1, 6),
                           vaccine_index_dose2 = 3L,
                           vaccine_index_booster = 5L,
                           vaccine_schedule = vaccine_schedule,
                           n_doses = 3L,
                           vacc_skip_to = c(0, 4L, 0, 6L, 0, 0),
                           vacc_skip_weight = c(0, 0.5, 0, 1, 0, 0),
                           vacc_skip_progression_rate = rep(0, 6))
  expect_equal(p$vacc_skip_dose, c(0L, 2L, 4L))
  expect_equal(p$vacc_skip_dose_inverse, c(0L, 2L, 0L, 3L, 0L, 0L))
  expect_equal(p$vacc_skip_dose_weight, c(0, 0.5, 1))
  expect_equal(p$vacc_skip_from, c(0L, 0L, 0L, 2L, 0L, 4L))
  expect_equal(p$vacc_skip_progression_rate_base, rep(0, 6))
  expect_equal(p$vacc_skipped, c(0L, 2L, 2L, 4L, 4L, 0L))
  ...
  p <- ZamCovid_parameters(1, "london",
                           ...
                           vaccine_schedule = vaccine_schedule,
                           n_doses = 3L,
                           vacc_skip_to = c(0L, 4L, 0L, 0L, 0L),
                           vacc_skip_weight = rep(0, 5),
                           vacc_skip_progression_rate = c(0, 2, 0, 0, 0))
  expect_equal(p$vacc_skip_dose, integer(3))
  expect_equal(p$vacc_skip_dose_inverse, integer(5))
  expect_equal(p$vacc_skip_dose_weight, rep(0, 3))
  expect_equal(p$vacc_skip_from, c(0L, 0L, 0L, 2L, 0L))
  expect_equal(p$vacc_skip_progression_rate_base, c(0, 2, 0, 0, 0))
  expect_equal(p$vacc_skipped, c(0L, 2L, 2L, 0L, 0L))
})
...
test_that("N_tots stay constant with vaccine skip moves", {
  ...
  p <- ZamCovid_parameters(1, "london",
                           ...
                           n_doses = 3,
                           vacc_skip_to = c(0L, 0L, 5L, 0L, 0L),
                           vacc_skip_weight = c(0, 0, 1, 0, 0),
                           vacc_skip_progression_rate = rep(0, 5))
...
test_that("Vaccine skip works in all compartments", {
  ...
  p <- ZamCovid_parameters(1, "london", beta_value = 0,
                           m_CHR = 0, m_CHW = 0,
                           waning_rate = 0,
                           rel_susceptibility = rep(1, 5),
                           vaccine_index_dose2 = 2L,
